...
/** Add multiple product IDs to a board in one write — prevents duplicates. */
export function addProductsToBoard(boardId, productIds) {
  const updated = loadBoards().map((b) =>
    b.id === boardId
      ? { ...b, items: [...new Set([...b.items, ...productIds])] }
      : b
  );
  saveBoards(updated);
  return updated;

 ...
 *
 * Owns the 4-step DNA quiz data, scoring engine, and persistence.
...
// ─── Step 1: Style vibes (image-based, multi-select) ─────────────────────────
...
// ─── Step 3: Colour palettes (grouped, not individual swatches) ───────────────
 ...
 * Uses a four-signal scoring pass:
 *   1. Style vibes   → direct style type match (+3 each)
 *   2. Outfits       → weighted per-outfit scores
 *   3. Colour groups → expanded to individual colours, scored via COLOUR_STYLE_MAP
 *   4. Brands        → scored via BRAND_STYLE_MAP
 *
...
export function computeDNA({
  ...
  // 3. Colour palette selections → expand to individual colour names
  ...
  return {
    ...
    // ── Structured quiz data (for display + future use) ───────────────────────
    styles,          // raw style vibe IDs from step 1
    outfits,         // raw outfit IDs from step 2
    paletteGroups: colours, // raw colour group IDs from step 3
    paletteType,     // display label of first selected colour group
    scores,          // full style scores (for debugging / insights)

    // ── Pre-built filter object (for shop page direct use) ────────────────────
    filters: {
      brands,
